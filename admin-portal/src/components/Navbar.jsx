...
const Navbar = () => {
    ...
    return (
        <header className="bg-white border-b border-gray-200 px-6 h-14 flex items-center justify-between sticky top-0 z-40 shadow-sm">
            {/* Brand */}
            <Link to="/dashboard" className="flex items-center gap-2 font-black text-base text-indigo-700 tracking-tight">
            ...
            {user?.role === 'ADMIN' && (
                <nav className="flex items-center gap-1">
                    ...
                    {navLink('/internships/new', <Plus size={14} />, 'New Internship')}
                    {navLink('/rejected', <XCircle size={14} />, 'Rejected')}
                    <button
                        onClick={logout}
