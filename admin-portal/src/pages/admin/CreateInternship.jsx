...
const CreateInternshipForm = () => {
    ...
    const availableDocs = [
        ...
        { id: 'NOC_LETTER', label: 'NOC / Principal Letter' },
                        ...
                        <div className="pt-4 border-t border-gray-100">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {availableDocs.map(doc => (
                                    <label key={doc.id} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${requiredDocs.includes(doc.id) ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-200' : 'bg-gray-50 border-gray-100 hover:border-gray-200'}`}>
                                        <input
                                            type="checkbox"
                                        ...
                                        />
                                        <span className={`text-sm font-semibold ${requiredDocs.includes(doc.id) ? 'text-indigo-900' : 'text-gray-600'}`}>{doc.label}</span>
                                    </label>
                                ))}
                            </div>
